...
void display_draw_status_bar(const char *message, uint16_t color) {
    // Assuming the status bar is at the bottom of the display
    // This fills a rectangle at the bottom and then writes text over it
    int status_bar_height = 20; // Height of the status bar
    int text_margin = 5;        // Margin for the text inside the status bar

    display_draw_rectangle(0, DISPLAY_HEIGHT - status_bar_height, DISPLAY_WIDTH, DISPLAY_HEIGHT,
                           color, true, 1);
    display_draw_string(text_margin, DISPLAY_HEIGHT - status_bar_height + text_margin, message,
                        &Font12, color, WHITE);
}

void display_clear_status_bar() {
    // Clears the status bar area by drawing a rectangle in the background color
    display_draw_rectangle(0, DISPLAY_HEIGHT - 20, DISPLAY_WIDTH, DISPLAY_HEIGHT, WHITE, true, 1);
}
